...
def checkstate(data):
    ...
    if int(data['co2'])>=1000 and int(data['co2'])<1500:
        problem.append("co2Warning")
        problemdata.append("co2")
    elif int(data['co2'])>=1500:
        problem.append("co2Danger")
        problemdata.append("co2")    
    
    if data['temperature']< 18:
        if data['humidity'] <61: 
            problem.append("too Dry")
            problemdata.append("humidity")
        elif data['humidity'] >80:
            problem.append("too Humid")
            problemdata.append("humidity")
    elif data['temperature']<21:
        if data['humidity'] <70: 
            problem.append("too Dry")
            problemdata.append("humidity")
        elif data['humidity'] >80:
    ...
    if data['temperature']>100:
        problem.append("too Hot")
        problemdata.append("temperature")
    if data['temperature']<0:
        problem.append("too cold")
        problemdata.append("temperature")
    ...
    print(len(problem))
    if len(problem) != 0:
        sql='INSERT INTO problendata (Room,) values (%s,%s,"%s",%s,"%s",%s,"%s",%s,"%s",%s,"%s")'
        print(sql)
    else: 
        print("없어요")   
    print(problem)
